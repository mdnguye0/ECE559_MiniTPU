module tpu_top #( 
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (start_exec) begin 
                act_pump_active <= 1'b1;
                act_sram_raddr <= '0; 
            end 
            if (act_sram_raddr == 10'd15 && act_sram_ren) begin
                act_pump_active <= 1'b0;
            end 

            if (act_pump_active && !act_fifo_full) begin
                act_sram_ren <= 1'b1;
            end else begin
                act_sram_ren <= 1'b0;
            end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin 
            ...
            act_valid_q <= act_fifo_rd_en; 
            if (act_fifo_rd_en) begin
                flush_timer <= 6'd48; 
                skew_buffer_en <= 1'b1;
            end else if (flush_timer > 0) begin
    ...
    generate 
        for (i = 0; i< N; i++) begin: UNPACK_ACT
            assign act_skew_in[i] = act_valid_q ? act_fifo_rdata[(i*ACT_WIDTH) +: ACT_WIDTH] : '0;        
        end
    endgenerate 
